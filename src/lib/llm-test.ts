...
function isEvolinkNativeConfig(config: LLMSettings): boolean {
	const baseUrl = config.baseUrl.toLowerCase()
	const model = config.model.toLowerCase()
	return baseUrl.includes('direct.evolink.ai') || (baseUrl.includes('evolink.ai') && model.includes('gemini'))
}

async function testEvolinkNativeConnection(config: LLMSettings): Promise<TestResult> {
	const baseUrl = config.baseUrl.replace(/\/+$/, '')
	const endpoint = `${baseUrl}/v1beta/models/${encodeURIComponent(config.model)}:generateContent`
...
export async function testLLMConnection(config: LLMSettings): Promise<TestResult> {
	if (isEvolinkNativeConfig(config)) {
		return testEvolinkNativeConnection(config)
	}
